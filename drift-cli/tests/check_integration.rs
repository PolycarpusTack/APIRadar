...
/// Path to the compiled `drift` binary.
fn drift_bin() -> std::path::PathBuf {
    // When integration tests run, the binary is already built by cargo test.
    // CARGO_BIN_EXE_drift is set automatically by cargo for [[bin]] targets.
