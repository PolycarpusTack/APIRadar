...
function resolveApiBinary(): string | null {
  ...
  const resourcesBin = join(process.resourcesPath ?? '', 'drift-api')
  if (existsSync(resourcesBin)) {
    return resourcesBin
  }
  const resourcesBinExe = join(process.resourcesPath ?? '', 'drift-api.exe')
...
function startApiSidecar(): ChildProcess | null {
  ...
  if (!bin) {
    ...
    console.warn(
      '[main] drift-api binary not found — falling back to `cargo run`. ' +
        'Run `cargo build -p drift-api` to avoid this slow start.'
    )
    try {
      const cargoProcess = spawn('cargo', ['run', '--bin', 'drift-api', '--', ...args], {
        cwd: workspaceRoot,
