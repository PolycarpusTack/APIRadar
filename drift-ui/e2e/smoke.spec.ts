// D-8: Smoke tests for drift-ui dashboard.
...
test.describe('Dashboard smoke tests', () => {
  ...
  test('health endpoint returns ok (API reachability)', async ({ request }) => {
    const apiUrl = process.env.DRIFT_API_URL ?? 'http://localhost:8080';
    ...
    if (resp.ok()) {
      ...
      test.skip(true, 'drift-api not reachable, skipping API health check');
    }
  });
});
